...
const AddWordDialog = ({ onWordAdded }: AddWordDialogProps) => {
  ...
  const fillWithGemini = async () => {
    ...
    setLoading(true);
    try {
      const prompt = `Ты — эксперт по ивриту. КРИТИЧЕСКИ ВАЖНЫЕ ПРАВИЛА:
1. Запрещено переводить слова поодиночке. Перевод строится на анализе ВСЕГО контекста (Context-Aware Translation).
2. Перед выводом определи часть речи каждого слова.
3. Учитывай ивритские омографы: если слово графически совпадает с глаголом прош. вр. (напр. מדד — «измерил»), но перед ним стоит определённый артикль ה (המדד), — это существительное («показатель / индекс»), а НЕ глагол. Аналогично для других пар (הספר = «книга», а не «считал»; המשפט = «предложение/суд», а не «судил»).
